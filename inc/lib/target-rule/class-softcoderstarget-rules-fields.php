 ...
 * Meta Boxes setup
 */
class softcoderselements_Target_Rules_Fields {
	/**
	 * Instance
	 *
	 * @since  1.0.0
	...
	public static function get_location_selections() {
		...
		$special_pages = array(
			'special-404'    => __( '404 Page', 'SoftCoders-header-footer-elementor' ),
			'special-search' => __( 'Search Page', 'SoftCoders-header-footer-elementor' ),
			'special-blog'   => __( 'Blog / Posts Page', 'SoftCoders-header-footer-elementor' ),
			'special-front'  => __( 'Front Page', 'SoftCoders-header-footer-elementor' ),
			'special-date'   => __( 'Date Archive', 'SoftCoders-header-footer-elementor' ),
			'special-author' => __( 'Author Archive', 'SoftCoders-header-footer-elementor' ),
		);

		if ( class_exists( 'WooCommerce' ) ) {
			$special_pages['special-woo-shop'] = __( 'WooCommerce Shop Page', 'SoftCoders-header-footer-elementor' );
		}

		$selection_options = array(
			'basic'         => array(
				'label' => __( 'Basic', 'SoftCoders-header-footer-elementor' ),
				'value' => array(
					'basic-global'    => __( 'Entire Website', 'SoftCoders-header-footer-elementor' ),
					'basic-singulars' => __( 'All Singulars', 'SoftCoders-header-footer-elementor' ),
					'basic-archives'  => __( 'All Archives', 'SoftCoders-header-footer-elementor' ),
				),
			),

			'special-pages' => array(
				'label' => __( 'Special Pages', 'SoftCoders-header-footer-elementor' ),
				'value' => $special_pages,
			),
		);
		...
		$selection_options['specific-target'] = array(
			'label' => __( 'Specific Target', 'SoftCoders-header-footer-elementor' ),
			'value' => array(
				'specifics' => __( 'Specific Pages / Posts / Taxonomies, etc.', 'SoftCoders-header-footer-elementor' ),
			),
		);
	...
	public static function get_user_selections() {
		$selection_options = array(
			'basic'    => array(
				'label' => __( 'Basic', 'SoftCoders-header-footer-elementor' ),
				'value' => array(
					'all'        => __( 'All', 'SoftCoders-header-footer-elementor' ),
					'logged-in'  => __( 'Logged In', 'SoftCoders-header-footer-elementor' ),
					'logged-out' => __( 'Logged Out', 'SoftCoders-header-footer-elementor' ),
				),
			),

			'advanced' => array(
				'label' => __( 'Advanced', 'SoftCoders-header-footer-elementor' ),
				'value' => array(),
			),
		);
	...
	public function admin_styles() {
		...
		$localize_vars = array(
			'ast_lang'      => $ast_lang,
			'please_enter'  => __( 'Please enter', 'SoftCoders-header-footer-elementor' ),
			'please_delete' => __( 'Please delete', 'SoftCoders-header-footer-elementor' ),
			'more_char'     => __( 'or more characters', 'SoftCoders-header-footer-elementor' ),
			'character'     => __( 'character', 'SoftCoders-header-footer-elementor' ),
			'loading'       => __( 'Loading more results…', 'SoftCoders-header-footer-elementor' ),
			'only_select'   => __( 'You can only select', 'SoftCoders-header-footer-elementor' ),
			'item'          => __( 'item', 'SoftCoders-header-footer-elementor' ),
			'char_s'        => __( 's', 'SoftCoders-header-footer-elementor' ),
			'no_result'     => __( 'No results found', 'SoftCoders-header-footer-elementor' ),
			'searching'     => __( 'Searching…', 'SoftCoders-header-footer-elementor' ),
			'not_loader'    => __( 'The results could not be loaded.', 'SoftCoders-header-footer-elementor' ),
			'search'        => __( 'Search pages / post / categories', 'SoftCoders-header-footer-elementor' ),
	...
	public static function target_rule_settings_field( $name, $settings, $value ) {
		...
		$add_rule_label = isset( $settings['add_rule_label'] ) ? $settings['add_rule_label'] : __( 'Add Rule', 'SoftCoders-header-footer-elementor' );
		$saved_values   = $value;
		$output         = '';
		...
		$output .= '<option value="">' . __( 'Select', 'SoftCoders-header-footer-elementor' ) . '</option>';
	...
	public static function get_post_target_rule_options( $post_type, $taxonomy ) {
		...
		$all_posts                          = sprintf( __( 'All %s', 'SoftCoders-header-footer-elementor' ), $post_label );
		$post_option[ $post_name . '|all' ] = $all_posts;

		if ( 'pages' != $post_key ) {
			/* translators: %s post label */
			$all_archive                                = sprintf( __( 'All %s Archive', 'SoftCoders-header-footer-elementor' ), $post_label );
		...
		if ( in_array( $post_type->name, $taxonomy->object_type ) ) {
			...
			$tax_archive = sprintf( __( 'All %s Archive', 'SoftCoders-header-footer-elementor' ), $tax_label );
	...
	public static function generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
		...
		if ( 'display' == $type ) {
			...
			$output .= '<a href="#" class="button">' . __( 'Add Exclusion Rule', 'SoftCoders-header-footer-elementor' ) . '</a>';
			$output .= '</div>';
		}
		...
		if ( ! empty( $already_set_rule ) ) {
			add_action(
				...
				function() use ( $already_set_rule ) {
					...
					$notice = sprintf( __( 'The same display setting is already exist in %s post/s.', 'SoftCoders-header-footer-elementor' ), $rule_set_titles );
	...
	public static function get_post_selection( $post_type ) {
		...
		if ( ! empty( $all_headers ) ) {
			$headers = array(
				'' => __( 'Select', 'SoftCoders-header-footer-elementor' ),
			);
 ...
 */
softcoderselements_Target_Rules_Fields::get_instance();
