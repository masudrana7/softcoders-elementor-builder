...
class Global_Theme_Compatibility {
	...
	public function hooks() {
		if ( socoders_elements_header_enabled() ) {
			...
			add_action( 'socoders_elements_fallback_header', [ 'Header_Footer_Elementor', 'get_header_content' ] );
		}

		if ( socoders_elements_is_sidebar_canvas_enabled() ) {
			add_action( 'get_header', [ 'get_header_content', 'get_sidebar_canvas_content' ], 15 );
		}
		...
		if ( socoders_elements_header_enabled() || socoders_elements_footer_enabled() ) {
	...
	public function force_fullwidth() {
		...
		if ( true === socoders_elements_header_enabled() ) {
			$css .= 'header#masthead {
				display: none;
			}';
		}

		if ( true === socoders_elements_footer_enabled() ) {
			$css .= 'footer#colophon {
				display: none;
			}';
		}

		wp_add_inline_style( 'sce-style', $css );
	}
	...
	public function option_override_header() {
		...
		if ( ! did_action( 'wp_body_open' ) ) {
			...
			do_action( 'socoders_elements_fallback_header' );
			echo '</div>';
		}
	}
}
